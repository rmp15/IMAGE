...
# fix where London should be on the map


# create a graticule from grid points for Europe
grat <- graticule(lons=grid$lon,lats=grid$lat,xlim=c(min(grid$lon),max(grid$lon)),ylim=c(min(grid$lat),max(grid$lat)),proj=projection(original.proj))

# plot fortified europe map
ggplot() +
    geom_text(data=grat,aes(x=lon,y=lat),size=3) +
