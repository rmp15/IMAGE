...
function mv=CLaGARM_noHash(nYears,nsp,mv,hashname)
tic; st=[]; %init section timer for toc_section calls

%% Model options
PREDESEAS=0; %pre transform deseason
DESEASON=1; % post transform deseason
ARCOEFF=1; % 0-1, Weight to generated AR coeffs. 0 is mean. 1 is generated.
...
disp('Transforming');
[opz,oz_dat]=mloc_nscore(op,'linlim'); %should try paretotails ecdf method
...
% uses hash value to determine whether fitting has been performed previously
%h=DataHash({opzbd,nsp,'calc_AR1_per_loc_sp_par'});
h=[hashname '_ARfit'];
...
for i=1:5
    ...
    %h=DataHash({c,p,nYears,'MVAREOF'});
    h=[hashname '_ARgen'];
    if i==1
        if ~exist([hdir h '.mat'],'file')
...
% plot(15+30.5*[0:11],mu(:,44,10));
        ...
        mus=ARCOEFF.*musp+(1-ARCOEFF).*mum;
    ...
    [rsz,SIG]=MVNgen_iter(rz,SIG,o,s,nYears);
    rs=mloc_inscore(rsz,rz_dat);
    %end
