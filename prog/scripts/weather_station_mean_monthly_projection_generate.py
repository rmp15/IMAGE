...
data = pd.read_csv(file_paths[0], delimiter=';', skiprows=14,decimal=",")#,columns = col_names)
...
for name in glob.glob(os.path.join(file_paths[0], metric + '_mean_scale_factors_' + str(year_past_start) + '_' +
    ...
    print(file_output)
    knmi_scenarios_apply_scale_factors(metric, data, file_paths[0], file_output, str(years_future_2[0]))
