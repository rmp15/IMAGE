function [r,T] = mvnrnd_eigs_save(mu,sigma,savedir,cases,T)
...
if ndims(sigma) == 2
    ...
    elseif sigmaIsDiag
        ...
        r = bsxfun(@times,randn(n,d,'like',outtype),t) + mu;
    else
        ...
        if ~exist([savedir 'cholcovTerr.mat'])
            disp('Calling cholcov_eigs')
            [T,err] = cholcov_eigs(sigma);
            mkdir(savedir)
            save([savedir 'cholcovTerr'],'T','err')
        else
            disp('loading precalc cholcov_eigs')
            load([savedir 'cholcovTerr']);
        end
...
elseif ndims(sigma) == 3
    ...
    if nargin < 4
        ...
        if sigmaIsDiag
            ...
            R = sqrt(sigma);
            r = bsxfun(@times,randn(n,d,'like',outtype),R) + mu;
            if nargout > 1
                for i=1:n
                    T(:,:,i) = diag(R(i,:));
                end
