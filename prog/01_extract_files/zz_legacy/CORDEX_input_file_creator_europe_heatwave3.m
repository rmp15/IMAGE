function output=CORDEX_input_file_creator_europe_heatwave3(variable_name,lbound,ubound,file_prefix,start_year,end_year,save_file_suffix)
...
for site = 1:no_of_gcs
    for year = 1:no_of_years
        ann_mean_vals(year) = mean(concatted_vari(:,year,site));
    end
    years = linspace(1,no_of_years,no_of_years);
    [r, b, c] = regression(years,ann_mean_vals)
    for year = 1:no_of_years
        concatted_vari(:,year,site) = concatted_vari(:,year,site) - b*year;
    end
end
...
save_file = strcat(variable_name,save_file_suffix,'detrend')
